...
class Network(nn.Module):
    ...
    def _proposal_layer(self, rpn_cls_prob, rpn_bbox_pred):
        ...
        return rois, rpn_scores  # pre_nms_topN/post_nms_topN * 5; pre_nms_topN/post_nms_topN * 1
    ...
    def _region_proposal(self, net_conv):
        ...
        if self._mode == 'TRAIN':
            ...
            if cfg.TEST.MODE == 'nms':
                ...
                rois, _ = self._proposal_layer(rpn_cls_prob,
                                               rpn_bbox_pred)  # rpn_cls_prob.shape = batch * h * w * (num_anchors * 2); rpn_bbox_pred.shape = batch * h * w * (num_anchors*4)
            elif cfg.TEST.MODE == 'top':
                rois, _ = self._proposal_top_layer(rpn_cls_prob, rpn_bbox_pred)
            else:
                raise NotImplementedError

        self._predictions["rpn_cls_score"] = rpn_cls_score  # rpn_cls_score 和 rpn_cls_score_reshape只是维度顺序不同, 内容是相同
        self._predictions["rpn_cls_score_reshape"] = rpn_cls_score_reshape
        self._predictions["rpn_cls_prob"] = rpn_cls_prob  # rpn_cls_score_reshape过了softmax -> 变成了概率
        self._predictions["rpn_cls_pred"] = rpn_cls_pred  # 每个anchor中的预测结果 -> 前景还是背景
        self._predictions["rpn_bbox_pred"] = rpn_bbox_pred  # 预测的全部anchor的坐标位置
        self._predictions["rois"] = rois  # 暴露的最有可能的前 pre_nms_topN/post_nms_topN 个 anchor
    ...
    def forward(self, image, im_info, gt_boxes=None, mode='TRAIN'):
        ...
        if mode == 'TEST':
            ...
            # expand_as: 将stds扩展(复制)至和bbox_pred维度一致
            # -> expand_as 可以扩展任意维度, 但是需保证其他维度的一致性,
            # 如: (10) -> (14, 12, 10) / (1) -> (14, 12, 10)
